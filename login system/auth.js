...
class AuthSystem {
    constructor() {
        ...
        this.microsoftDomainRegex = /^[a-zA-Z0-9._%+-]+@mkhb\.moe\.gov\.sa$/;
        this.init();
    }
    ...
    handleRegister(e) {
        ...
        // Enforce Microsoft domain for students/teachers
        if ((userData.userType === 'student' || userData.userType === 'teacher') && !this.microsoftDomainRegex.test(userData.email)) {
            this.showNotification('يجب أن يكون البريد بصيغة mkhb.moe.gov.sa@ للمعلّمين والطلاب', 'error');
            return;
        }
    ...
    handleLogin(e) {
        ...
        // Enforce Microsoft domain for students/teachers on login
        if ((user.userType === 'student' || user.userType === 'teacher') && !this.microsoftDomainRegex.test(email)) {
            this.showNotification('يجب تسجيل الدخول ببريد mkhb.moe.gov.sa@ للطلاب والمعلمين', 'error');
            return;
        }
    ...
    // Reset password without old password (used by modal on login page)
    resetPassword(email, newPassword) {
        if (!email || !newPassword) {
            this.showNotification('يرجى إدخال البريد وكلمة مرور جديدة', 'error');
            return false;
        }

        const userIndex = this.users.findIndex(u => u.email === email);
        if (userIndex === -1) {
            this.showNotification('لم يتم العثور على مستخدم بهذا البريد', 'error');
            return false;
        }
        ...
        localStorage.setItem('users', JSON.stringify(this.users));
        this.showNotification('تم تحديث كلمة المرور بنجاح', 'success');
        return true;
    }

    // Simulate Microsoft sign-in (no external OAuth)
    signInWithMicrosoft(email) {
        if (!this.microsoftDomainRegex.test(email)) {
            this.showNotification('يرجى استخدام بريد من نطاق mkhb.moe.gov.sa@', 'error');
            return false;
        }

        // If user exists, login; otherwise create a student user by default
        ...
        if (!user) {
            user = {
                ...
                email: email,
                userType: 'student',
                password: this.hashPassword(this.generateId()),
                createdAt: new Date().toISOString(),
                lastLogin: null,
                profile: { avatar: null, phone: '', address: '', bio: '' }
            };
        ...
        this.showNotification('تم تسجيل الدخول عبر مايكروسوفت', 'success');
